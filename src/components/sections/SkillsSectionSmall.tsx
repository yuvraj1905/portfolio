...
const SkillsSectionSmall = () => {
  const { shortSkills:finalListOfSkills } = portfolioData;
  if (!finalListOfSkills || finalListOfSkills.length === 0) {
    return null;
  }

  return (
    <section id="skills" className="py-4 bg-transparent">
      <div className="flex flex-col items-center justify-center px-4 sm:px-6 lg:px-8 gap-4 sm:gap-6">
        <h2 className="text-xl sm:text-3xl font-bold text-center text-gray-400 ">
          My Tech Stack
        </h2>
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-7 xl:grid-cols-9 gap-x-4 gap-y-6 justify-items-center xl:gap-x-2">
                {finalListOfSkills.map((skill, skillIndex) => (
                  <div 
                    key={skillIndex} 
